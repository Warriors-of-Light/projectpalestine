  ...
  const handleForm: React.FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (isRegisteration) {
      if (password !== confirmPassword) {
        alert(t("login.password-mismatch",{defaultValue:"password mismatch"} ));
      } else {
        ...
        alert(t("login.sign-up-success",{defaultValue:"Sign up successful!"} ));
        setUser(result);
      ...
      <div className="flex flex-col w-full max-w-[550px] h-full justify-center">
        ...
        <div className="bg-app--light shadow-lg md:rounded-xl flex flex-col gap-6 p-6 border-2 border-app-primary">
          <h2 className="text-4 text-app--primary text-center">
            {t("header.project-name",{defaultValue:"Project Palestine"} )}
          </h2>
          ...
          <form className="space-y-6" action="#" onSubmit={handleForm}>
            ...
            <div>
              ...
              <input
                ...
                type="email"
                autoComplete="email"
                placeholder={t("login.email-placeholder", {defaultValue:"free@palestine.com"} )}
                required
            ...
            {isRegisteration && (
              <div>
                <label className="flex text-green-800 text-lg font-semibold mb-2">
                {t("login.confirm-password",{defaultValue:"Confirm Password"} )}
                </label>
                <input
                  id="confirm-password"
            ...
            <div className="flex flex-col items-center gap-4">
              <button type="submit" className="app-btn w-1/2">
                {isRegisteration ? t("login.sign-up",{defaultValue:"Sign up"} ) : t("login.login-button",{defaultValue:"Login"} )}
              </button>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-app--primary">{t("login.or",{defaultValue:"Or"} )}</span>
                <hr className="w-full border-t border-app--primary" />
              </div>
              <div className="text-center">
                {isRegisteration ? t("login.already-account",{defaultValue:"Already a member?"} ) : t("login.no-account",{defaultValue:"Not a member?"} )} {"  "}
                <Link
                  href="#"
                  className="app-link"
                  onClick={() => setIsRegisteration(!isRegisteration)}
                >
                  {isRegisteration ? t("login.sign-in",{defaultValue:"Sign in"} ) : t("login.sign-up",{defaultValue:"Sign up"} )}
                </Link>
              </div>
            </div>
          </form>
